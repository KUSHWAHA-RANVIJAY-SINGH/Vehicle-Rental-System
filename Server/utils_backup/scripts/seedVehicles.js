...
const vehicles = [
  ...
  return {
    ...
    color: 'Black',
    registrationNumber: `MH-${Math.floor(100 + Math.random() * 900)}-AB-${Math.floor(1000 + Math.random() * 9000)}`,
    vin: `${Math.random().toString(36).substring(2, 19).toUpperCase()}`,
    odometerKm: Math.floor(Math.random() * 80000),
    fuelEconomy: parseFloat((Math.random() * 15 + 10).toFixed(1)),
